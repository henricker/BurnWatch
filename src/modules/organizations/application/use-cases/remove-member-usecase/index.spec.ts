...
function createPrismaMock(overrides: {
  ...
  return {
    profile: { findUnique, findFirst, delete: deleteProfile },
...
describe("RemoveMemberUseCase", () => {
  ...
  it("throws MemberError when profile to remove does not exist", async () => {
    ...
    await expect(
      useCase.execute({ requesterUserId, profileIdToRemove }),
  ...
  it("throws MemberForbiddenError when requester is MEMBER", async () => {
    const prisma = createPrismaMock({
      targetProfile: {
        id: profileIdToRemove,
        userId: "other-member",
  ...
  it("deletes profile when OWNER removes a MEMBER and returns removedUserId", async () => {
    ...
    const useCase = new RemoveMemberUseCase(prisma);
    const result = await useCase.execute({ requesterUserId, profileIdToRemove });
    expect(result).toEqual({ removedUserId: "member-uuid" });
    expect(deleteProfile).toHaveBeenCalledWith({ where: { id: profileIdToRemove } });
  });
