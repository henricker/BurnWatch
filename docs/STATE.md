...
- **Modelos principais**
  - `Organization`
    ...
    - Campos principais: `id (UUID)`, `name`, `createdAt`, `updatedAt`, `slackWebhookUrl`, `discordWebhookUrl`, `notificationSettings` (Json: `{ anomaly?: boolean, dailySummary?: boolean, limitWarning?: boolean }`).
...
- **getDashboardAnalytics (GetDashboardAnalyticsUseCase):** mock de `prisma.dailySpend.findMany` e `aggregate` com `vi.useFakeTimers` para data fixa; casos: zero rows (totais zero, evolution com 5 dias, forecastCents 0 para MTD); agregação totalCents e trendPercent; MTD forecastCents = (totalMTD/daysElapsed)*totalDays; 7D sem forecast; evolution com aws/vercel/gcp e labels; providerBreakdown ordenado por total com services; categorias a partir de serviceName (Compute, Database); filtro por provider (where.provider). **anomalyDetails:** agrupamento por (provider, serviceName) com histórico vs "hoje"; mesma regra do TriggerAnomalyAlertAfterSync (Z-Score > 2, > $1, spike > 20%); retorno `anomalyDetails: AnomalyDetail[]` (provider, serviceName, currentSpend, averageSpend, spikePercent, zScore) para a UI listar todas as anomalias por provedor; `anomalies` passa a refletir o tamanho de `anomalyDetails` quando não vazio.
...
### Módulo adapter-engine (Milestone 04 + 06 + 07)

- **Domínio** (`src/modules/adapter-engine/domain/cloudProvider.ts`): interface `ICloudProvider` (`fetchDailySpend(cloudAccount, range)`), tipos `DailySpendData`, `FetchRange`, constantes `SYNC_ERROR_VERCEL_FORBIDDEN`, `SYNC_ERROR_AWS_INVALID_CREDENTIALS`, `SYNC_ERROR_GCP_INVALID_CREDENTIALS`, `SYNC_ERROR_GCP_BILLING_EXPORT` e classe `SyncErrorWithKey` para erros com chave armazenável em `lastSyncError`.
- **VercelProvider** (`src/modules/adapter-engine/infrastructure/providers/vercelProvider.ts`): integração real com Vercel Billing API (`GET /v1/billing/charges`), desencriptação do token, resposta JSONL normalizada para `amountCents` e `serviceName`; em 403 com `invalidToken`/“Not authorized” lança `SyncErrorWithKey` com chave `vercel-forbidden-error-sync`. Possui modo fake via `fakeVercelBilledResponse(from, to)` activado por `USE_FAKE_VERCEL_BILLING="true"` (JSONL com serviços típicos da Vercel). **Simulação de anomalias:** `ANOMALY_VERCEL_ACTIVE="true"` inflaciona o dia "hoje" no fake. Helper partilhado em `util/anomalySimulation.ts` (`isAnomalySimulationEnabled`, `getAnomalySpikeMultiplier`, `isTodayDate`); testes em `anomalySimulation.spec.ts`.
- **AwsProvider** (`src/modules/adapter-engine/infrastructure/providers/awsProvider.ts` – Milestone 06): integração com AWS Cost Explorer via `@aws-sdk/client-cost-explorer` (`GetCostAndUsageCommand` com `Granularity: "DAILY"`, `Metrics: ["UnblendedCost"]`, `GroupBy` por `SERVICE`). Desencripta `accessKeyId`/`secretAccessKey`/`region` do `encryptedCredentials`, normaliza `Amount` (string decimal) para `amountCents` (inteiro) e usa `serviceName` = nome do serviço AWS. Em erros de credencial (`InvalidClientTokenId`, `InvalidSignatureException`, etc.) lança `SyncErrorWithKey` com chave `aws-invalid-credentials-error`. Também expõe `fakeAwsBilledResponse(range)` para uso local, controlado por `USE_FAKE_AWS_BILLING="true"`. **Simulação de anomalias:** quando `ANOMALY_AWS_ACTIVE="true"`, o fake inflaciona os valores do dia "hoje" (multiplicador `ANOMALY_SPIKE_MULTIPLIER`, default 5) para testar detecção e notificações sem custo real.
- **GcpProvider** (`src/modules/adapter-engine/infrastructure/providers/gcpProvider.ts` – Milestone 07): credenciais Service Account JSON (`project_id`, `private_key`, `client_email`); ingestão via **BigQuery Billing Export** (tabela `gcp_billing_export_v1_<<BILLING_ACCOUNT_ID>>`); modo fake `USE_FAKE_GCP_BILLING="true"` (Compute Engine, BigQuery, Cloud Run, Cloud Storage); erros `gcp-invalid-credentials-error` e `gcp-billing-export-error`; testes em `gcpProvider.spec.ts`. **Simulação de anomalias:** `ANOMALY_GCP_ACTIVE="true"` inflaciona o dia "hoje" no fake.
- **MockProvider** (`src/modules/adapter-engine/infrastructure/providers/mockProvider.ts`): retorna `[]` para `OTHER`.
- **SyncAccountUseCase** (`src/modules/adapter-engine/application/use-cases/sync-account-usecase/index.ts`): orquestra sync por conta; para `GCP` usa `GcpProvider`, para `AWS` usa `AwsProvider`, para `VERCEL` usa `VercelProvider`, demais `MockProvider`.
- **Conexões (UI):** Em SYNC_ERROR com `lastSyncError`, tooltip na célula de estado mostra mensagem traduzida (chaves `syncErrorVercelForbidden`, `syncErrorAwsInvalidCredentials`, `syncErrorGcpInvalidCredentials`, `syncErrorGcpBillingExport` em pt/en/es) ou texto bruto. Estado “A sincronizar…” tem prioridade (mostrado assim que o utilizador clica em sync); `syncingIds` é limpo ao receber resposta da API. **Ao criar um cloud provider:** a conta é adicionada já com `status: "SYNCING"` e o id em `syncingIds` no `onSuccess` do modal; condição unificada `showSyncing = isSyncing || acc.status === "SYNCING"` para a coluna “Último sync” (ícone + “Sincronizando…”), botão de sync (disabled + spin) e célula de estado — assim a primeira renderização já mostra loading em toda a linha. Token Vercel aceite em formato alfanumérico na validação do formulário.
...
### Módulo notifications (Milestone 08)

- **Domínio:** `src/modules/notifications/domain/` – tipos `ServiceAnomaly`, `ProviderAnomalyGroup`, `MultiCloudAnomalyReport`, `NotificationSettings`; interface `INotificationProvider` (sendAnomalyAlert, sendTestMessage); `getOwnerLocale(prisma, organizationId)` para resolver o locale do OWNER; `notificationMessages.ts` com copy para Slack e Discord em pt/en/es (anomalyDescriptionConsolidated, monthlyContextLabel, budgetSpendLine, consumedLabel, anomalyFooter, etc.).
- **Infraestrutura:** `SlackProvider` e `DiscordProvider` implementam `INotificationProvider`; todas as frases vêm de `getNotificationMessages(locale)` (nenhum texto chumbado); payloads com Block Kit (Slack) e embeds (Discord).
- **Use cases:** `SendAnomalyAlertUseCase(prisma, slack?, discord?)` – carrega org, resolve locale do owner, envia report consolidado (MultiCloudAnomalyReport) para webhooks configurados quando `notificationSettings.anomaly` é true; `TestWebhookConnectionUseCase` – envia mensagem de teste (aceita `webhookUrl` opcional no body para testar sem gravar); `TriggerAnomalyAlertAfterSyncUseCase(prisma, sendAnomalyAlert?)` – após sync, busca dailySpend dos últimos 14 dias, agrupa por provider/serviço, aplica regra de anomalia (Z-Score > 2, valor > $1, spike > 20%), constrói report e chama SendAnomalyAlertUseCase; erros são engolidos para não afetar o fluxo de sync.
- **API:** `GET/PATCH /api/notifications` (ler/atualizar webhooks e notificationSettings), `POST /api/notifications/test` (testar webhook, opcionalmente com URL no body).
- **UI:** `/dashboard/notifications` – inputs Slack/Discord, botão "Test" (só ativo com URL preenchida), botão "Save" (só ativo quando valor difere do gravado); teste sem gravar suportado.
- **Trigger pós-sync:** Em `POST /api/cloud-accounts/[id]` (sync), quando o resultado é SYNCED, `TriggerAnomalyAlertAfterSyncUseCase` é executado em fire-and-forget (`.catch(() => {})`).

Resultado atual: `pnpm test` passa com **147 testes** (incl. encryption, dailySpend, organizations, cloud-provider-credentials, adapter-engine (SyncAccountUseCase + AwsProvider, VercelProvider, GcpProvider), analytics, **notifications** (SlackProvider, DiscordProvider, SendAnomalyAlertUseCase, TestWebhookConnectionUseCase, TriggerAnomalyAlertAfterSyncUseCase)).
...
- **Criptografia**
  - `ENCRYPTION_KEY` – chave de 32 bytes:
    ...
    - 64 chars hex.
- **Modo fake (opcional, para testes/desenvolvimento sem chamadas reais)**
...
**Milestone 06.5 concluído – Backend Architecture Improvements (Arch Improvements):** Use cases em classes (dependências no construtor, `execute()`); layout em três camadas por módulo (domain / application/use-cases / infrastructure); uma pasta por use case em kebab-case com `index.ts` e `index.spec.ts` em todos os módulos; rotas API finas; Vitest com `include: ["src/**/*.test.ts", "src/**/*.spec.ts"]`. Necessário para a evolução sustentável da plataforma.

**Milestone 07 concluído – GCP Integration (The Data Lake):** GcpProvider em `src/modules/adapter-engine/infrastructure/providers/gcpProvider.ts` implementa `ICloudProvider`. Credenciais: Service Account JSON (após desencriptar, extrai `project_id`, `private_key`, `client_email`); validação e parse com erros `SyncErrorWithKey` (`gcp-invalid-credentials-error`, `gcp-billing-export-error`). Ingestão real via **BigQuery Billing Export** (query à tabela `gcp_billing_export_v1_<<BILLING_ACCOUNT_ID>>`, agregação por dia e `service.description`, conversão de custo para `amountCents`). Modo fake `fakeGcpBilledResponse(range)` com `USE_FAKE_GCP_BILLING="true"` (Compute Engine, BigQuery, Cloud Run, Cloud Storage). SyncAccountUseCase usa GcpProvider para `provider === "GCP"`. Tooltip em Connections com mensagens traduzidas para os erros GCP (pt/en/es). serviceNameToCategory com keyword "bigquery" → Database. Dashboard mantém cor #22c55e para GCP. Testes em `gcpProvider.spec.ts`.

**Milestone 08 concluído – Notification Engine:** Módulo `notifications` com domain (tipos MultiCloudAnomalyReport, ProviderAnomalyGroup, ServiceAnomaly; INotificationProvider; getOwnerLocale; notificationMessages pt/en/es), SlackProvider e DiscordProvider (mensagens 100% via locale, sem texto chumbado), SendAnomalyAlertUseCase, TestWebhookConnectionUseCase, TriggerAnomalyAlertAfterSyncUseCase (classe com execute, lógica de 14 dias + Z-Score + report consolidado, erros engolidos). API GET/PATCH /api/notifications e POST /api/notifications/test (teste com URL opcional no body). Página /dashboard/notifications com Test (só com URL) e Save (só quando valor difere). Trigger pós-sync em POST /api/cloud-accounts/[id] (fire-and-forget). 137 testes e build passando.
...
- **Dashboard – melhorias pós-M08:**
  - **Lista de anomalias por provedor:** API de analytics devolve `anomalyDetails` (GetDashboardAnalyticsUseCase aplica a mesma regra de anomalia por serviço: Z-Score > 2, valor > $1, spike > 20%); UI agrupa por AWS/GCP/Vercel com linhas expansíveis (chevron, clique para ver serviços anómalos e +XX%), ao estilo do bloco Detalhamento de Recursos; chaves `anomalyAlertSubtitle`, `anomalyOneService`, `anomalyManyServices` em pt/en/es.
  - **Loading com Skeleton:** Cards de métricas (StatCardSmall com `isLoading` e skeleton no valor), gráfico de evolução (eixo Y, barras skeleton, eixo X), Resource Breakdown (3 linhas skeleton), Gasto por Categoria (5 itens skeleton), banner de status/anomalias (círculo + 2 linhas skeleton). Componente shadcn `Skeleton` em todos os blocos.
  - **Cartão de Estado:** `valueVariant="healthy"` (verde esmeralda #10b981 + ponto) e `valueVariant="alert"` (vermelho + ponto); traduções `statusHealthy` / `statusAlert` em pt (Saudável, Alerta), es (Saludable, Alerta), en (Healthy, Alert).
  - **Troca de idioma sem refetch:** `fetchAnalytics` em `useCallback` com dependências apenas `[dateRange, providerFilter]`; mensagem de erro genérica guardada como chave `"errorLoading"` e traduzida na renderização (`t("errorLoading")`), para que a alteração de locale não dispare nova requisição a `/api/analytics`.
